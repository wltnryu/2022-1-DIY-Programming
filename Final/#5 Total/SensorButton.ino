...
void loop()
{
  ...
  while (client.available()) {
    ...
    if(ch == 0x0a) {
      ptr = strstr(line, "led=");
      if(ptr != NULL && ptr != "" ) {
        ptr = strcpy(ledstatus,(ptr+4)); // on, off 탐
...
void httpRequest()
{
  ...
  if (client.connect(server, 80)) {
    ...
    if (buttonState == 0) sprintf(SBuf, "GET /~yebbnrjs/SensorCont.jsp?temp=%s&moisture=%s&BUTTON=on HTTP/1.1", Hum, Temper); 
    else sprintf(SBuf, "GET /~yebbnrjs/SensorCont.jsp?temp=%s&moisture=%s&BUTTON=off HTTP/1.1", Hum, Temper);
